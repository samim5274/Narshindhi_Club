                ...
                <div class="table-responsive" style="max-height: 750px; overflow-y: auto;">
                    <table class="table table-bordered table-hover align-middle shadow-sm mb-0">
                        <thead class="table-dark">
                           <tr class="table-light">
                                <th colspan="4" class="text-center">
                                    <form action="{{ url('/filter-stock-out-date') }}" method="GET" class="d-flex justify-content-center align-items-center gap-2">
                        ...
                        <tbody id="food_table_body">
                            @foreach ($stockOutDetails as $key => $stockIn)
                                <tr>
                                    <td class="text-center">{{ $key + 1 }}</td>
                                    <td>{{ $stockIn->product->name ?? 'N/A' }}</td>
                                    <td class="text-center">{{ $stockIn->stockOut }}</td>
                                </tr>
                            @endforeach
                            <tr  class="table-secondary">
                                <td colspan="2">Total</td>
                                <td class="text-center">{{ $stockOutDetails->sum('stockOut') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
