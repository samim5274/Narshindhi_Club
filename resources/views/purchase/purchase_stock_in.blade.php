           ...
           @include('layouts.message')
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <ul class="breadcrumb">
                                ...
                                <li class="breadcrumb-item" aria-current="page">Purchases Stcok In</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div class="table-responsive">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5>Purchases Products Stock In</h5>
                </div>
                            ...
                            <table class="table table-bordered table-hover align-middle mb-0">
                                <thead class="table-dark position-sticky top-0">
                                    <tr>
                                        ...
                                        <th>Date</th>
                                        <th>Name</th>
                                        <th width="120" class="text-end">Stock In</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($stockInDetails as $val)
                                        <tr>
                                            <td class="text-center">{{ $loop->iteration }}</td>
                                            <td>{{ \Carbon\Carbon::parse($val->date)->format('d M, Y') }}</td>
                                            <td>{{ $val->product->name }}</td>
                                            <td class="text-end">{{ $val->stockIn }}</td>
                                        </tr>
                                    @empty
                                        <tr>
                                ...
                                <tfoot>
                                    <tr class="table-secondary fw-bold">
                                        <td colspan="3" class="text-center">Total Stock In:</td>
                                        <td class="text-end">{{ $stockInDetails->sum('stockIn') }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
    ...
    <div class="modal fade" id="product{{$val->id}}" tabindex="-1" aria-labelledby="editFoodLabel{{$val->id}}" aria-hidden="true">
        <div class="modal-dialog modal-lg"> <!-- Larger modal -->
            <div class="modal-content">
                <form action="{{ url('/product-stock-in/'.$val->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
